...
extension _CpfValidateScreenTypeExt on _CpfValidateScreenType {
  ...
  String label(BuildContext context) =>
      [AppLocalizations.of(context)!.cpfValidateScreenValid, AppLocalizations.of(context)!.cpfValidateScreenInvalid, ''][index];
}
...
class _CpfValidateScreenState extends State<CpfValidateScreen> {
  ...
  void initState() {
    ...
    maskFormatter = MaskTextInputFormatter(mask: '###.###.###-##', type: MaskAutoCompletionType.lazy);
    super.initState();
  }
                ...
                child: PtTextField(
                  ...
                  onChanged: (cpf) {
                    ...
                    if (invalidCpf.contains(cpf)) {
                      ...
                      type = valid ? _CpfValidateScreenType.VALID : _CpfValidateScreenType.INVALID;
                    }
                    setState(() {});
                  },
                ),
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      ...
                      for (var i = 9; i >= 1; i--) {
                        _controllerTextField.text = maskFormatter.maskText(_controllerTextField.text + Random().nextInt(9).toString());

                        await Future<void>.delayed(
                          const Duration(milliseconds: 100),
                        );
                      }
                      var generatedCpf = _generateCpf(_controllerTextField.text.split('')..removeWhere((element) => element == '.'));

                      _controllerTextField.text = maskFormatter.maskText(generatedCpf);
                      type = _CpfValidateScreenType.INITIAL;
                      setState(() {});
                    },
                    style: ButtonStyle(
                    ...
                    child: Text(AppLocalizations.of(context)!.cpfValidateScreenCPFGenerate),
                  ),
                ],
              )
            ],
